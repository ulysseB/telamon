...
fn clean_perfcounter_str(s: &str) -> String {
    //TODO(display bench) clean names
    s.replace("Subp", "")
        .replace("Sector", "Sct")
        .replace("read", "Rd")
        .replace("write", "Wr")
...
fn create_table(parameters: &[&str], counters: &[PerfCounter]) -> Table<u64> {
    let header = parameters
        ...
        .chain(
            counters
                .iter()
                .map(|x| clean_perfcounter_str(&x.to_string())),
        )
        .collect_vec();
    Table::new(header)
}
...
) -> Table<u64>
{
    ...
    let fun = match bench_choice {
        0 => {
            ...
            n_inst_per_iter = 5;
            cgen::incr_array(
                &base, gpu, ld_flag, st_flag, "n_outer", "n", "stride", mem_ids[0],
                "tab",
            )
        }
        1 => {
        ...
        2 => {
            ...
            n_inst_per_iter = 4;
            cgen::acc_array(
                &base, gpu, ld_flag, "n_outer", "n", "stride", mem_ids[0], "tab",
            )
        }
        3 => {
            n_access_per_iter = 1;
            n_inst_per_iter = 3;
            cgen::write_array(
                &base, gpu, st_flag, "n_outer", "n", "stride", mem_ids[0], "tab",
            )
        }
