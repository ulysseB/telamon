...
pub trait Kernel<'a>: Sized {
    ...
    /// Builds the signature of the kernel in the builder and returns an object
    /// that stores enough information to later build the kernel body and
    /// check its result. The `is_generic` flag indicates if th sizes
    ...
    /// Builder the kernel body in the given builder. This builder should be
    /// based on the signature created by `build_signature`.
    fn build_body<'b>(
        &self,
        ...
        while num_runs < num_tests {
            ...
            if let Some(leaf) = leaf {
                ...
                if let Err(err) = kernel.check_result(&expected_output, context) {
                    std::fs::write("actions_dump.txt", format!("{:?}", leaf.actions))
                        .expect("Could not write actions dump");
                    panic!(
    ...
    /// Tests the correctness of the bound of kernels and returns the list of
    /// tested leafs along with the actual evaluation time.
    fn test_bound<AM>(
        params: Self::Parameters,
        num_tests: usize,
        context: &mut AM,
    ...
    /// Computes the probability of encountering a dead-end when descending in
    /// the search tree.
    fn deadend_ratio<AM>(
        params: Self::Parameters,
        num_samples: usize,
...
/// Descend along a path in the search tree and stores the bounds encountered
/// on the way.
