    ...
    S: Scalar,
{
    ...
    fn build_body<'b>(
        ...
        let tiling = ::generate_tile_sizes(self.n as u32, 4096, 2);
        ...
        // TODO(search_space): try more tile sizes.
        ::limit_tile_size(&ld_x[0], &[1024, 4], &mut builder);
    ...
    S: Scalar,
{
    ...
    fn build_body<'b>(
        ...
        let m_tiling = ::generate_tile_sizes(self.m as u32, 2048, 2);
        let n_tiling = ::generate_tile_sizes(self.n as u32, 128, 1);

        let mut builder = Builder::new(&signature, ctx.device());
        let ld_x = self.x.load(vec![n_tiling.clone()], &mut builder);
        let ld_a = self.a.load(vec![m_tiling, n_tiling], &mut builder);
        // TODO(search_space): try more tile sizes.
        ::limit_tile_size(&ld_a[0], &[128, 16], &mut builder);
...
impl<'a, S: Scalar> Kernel<'a> for Gesummv<'a, S> {
    ...
    fn build_body<'b>(
        ...
        let ld_b = self.b.load(vec![m_tiling, n_tiling], &mut builder);
        // TODO(search_space): try more tile sizes.
        ::limit_tile_size(&ld_a[0], &[128, 16], &mut builder);
        let init_dim_m = builder.open_mapped_dim(&ld_a[0]);
...
impl<'a, S: Scalar> Kernel<'a> for MatMul<'a, S> {
    ...
    fn build_body<'b>(
        ...
        // FIXME: handle fixed tiles
        let k_tiling = ::generate_tile_sizes(self.params.k as u32, 64, 1);
        let m_tiling = ::generate_tile_sizes(self.params.m as u32, 512, 2);
        let n_tiling = ::generate_tile_sizes(self.params.n as u32, 512, 2);
        let mut builder = helper::Builder::new(signature, ctx.device());

        let ld_a = self.a.load(vec![m_tiling, k_tiling.clone()], &mut builder);
        let ld_b = self.b.load(vec![k_tiling, n_tiling], &mut builder);
        // TODO(search_space): try more tile sizes.
        ::limit_tile_size(&ld_a[0], &[64, 8], &mut builder);
        ::limit_tile_size(&ld_b[1], &[64, 8], &mut builder);
        ...
        // FIXME: apply tile sizes from the params
...
impl<'a, S: Scalar> Kernel<'a> for BatchMM<'a, S> {
    ...
    fn build_body<'b>(
        ...
        let m_tiling = ::generate_tile_sizes(self.params.m as u32, 64, 1);
        let n_tiling = ::generate_tile_sizes(self.params.n as u32, 64, 1);
        let k_tiling = ::generate_tile_sizes(self.params.k as u32, 64, 1);
        let batch_tiling = ::generate_tile_sizes(self.params.batch as u32, 128, 1);
