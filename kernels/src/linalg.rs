...
where S: Scalar
{
    ...
    ) -> Vec<Candidate<'b>>
    {
        ...
        tilings
            .into_iter()
            .map(|m_tiling| {
                let n_tiling = if m_tiling.len() > 0 {
                    vec![m_tiling[0]]
                } else {
                    vec![]
                };
                ...
                let a_op =
                    ld_a.dim_map(&[&acc_dim_m, &acc_dim_n], GlobalScope, &mut builder);
        ...
        tilings
            ...
            .map(|m_tiling| {
                ...
                let a_op =
                    ld_a.dim_map(&[&acc_dim_m, &acc_dim_n], GlobalScope, &mut builder);
                let b_op =
                    ld_b.dim_map(&[&acc_dim_m, &acc_dim_n], GlobalScope, &mut builder);
                let x_op = ld_x.dim_map(&[&acc_dim_n], GlobalScope, &mut builder);
    ...
    ) -> Vec<Candidate<'b>>
    {
        ...
        tilings
            .map(|((m_tiling, n_tiling), k_tiling)| {
                ...
                let a_op =
                    ld_a.dim_map(&[&acc_dim_m, &acc_dim_k], GlobalScope, &mut builder);
                let b_op =
                    ld_b.dim_map(&[&acc_dim_k, &acc_dim_n], GlobalScope, &mut builder);
                ...
                //builder.action(Action::InstFlag(ld_a.inst(), InstFlag::MEM_CG |
                // InstFlag::MEM_NC)); builder.action(Action::
                // InstFlag(ld_b.inst(), InstFlag::MEM_CG | InstFlag::MEM_NC));
...
impl BatchMMP {
    ...
    /// Generate code that is onyl valid for the given sizes. The batch size is
    /// still generic.
    pub fn static_sizes(mut self) -> Self {
        self.generic = false;
        self
