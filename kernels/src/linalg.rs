    ...
    S: Scalar,
{
    ...
    fn get_expected_output(&self, context: &device::Context) -> Array1<S> {
        ...
        self.a.read_to_host(context)
            .into_shape(a_shape)
            .unwrap()
            .dot(&self.x.read_to_host(context).into_shape(self.n as usize).unwrap())
    }
    ...
    fn check_result(
        ...
        let y = self.y.read_to_host(context).into_shape(self.m as usize).unwrap();
        if y.iter()
            .zip_eq(expected)
...
impl<'a, S: Scalar> Kernel<'a> for BatchMM<'a, S> {
    ...
    fn get_expected_output(&self, context: &device::Context) -> Array3<S> {
        ...
        let a = self.a.read_to_host(context).into_shape((batch, m, k)).unwrap();
        let b = self.b.read_to_host(context).into_shape((batch, k, n)).unwrap();
