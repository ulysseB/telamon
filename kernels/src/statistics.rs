...
impl std::fmt::Display for Estimate {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{value:.2e}{unit} (+/-{interval:.2e}{unit} @ {confidence:.0}%)",
               value = self.value, unit = self.unit, interval = self.interval,
...
pub fn estimate_mean(mut data: Vec<f64>, confidence: f64, unit: &'static str)
    -> Estimate
{
    ...
    Estimate { value: mean, unit, interval: data[idx], confidence }
}
...
pub fn estimate_ratio(ratio: f64, num_samples: usize) -> Estimate {
    ...
    Estimate { value: ratio, unit: "", interval, confidence: 0.95 }
}
