...
/// Creates a candidate from the search space and registers the tile sizes in
/// it.
...
/// Creates a `DimSize`. If the instantiate flag is true, it uses a constant
/// size, otherwise it creates a parameter with the given name.
fn create_size<'a, AM>(
    value: i32,
    name: &'a str,
    is_generic: bool,
    ...
    + std::ops::Neg<Output = Self>
{
    ...
    /// Indicates if the scalar can be considered as zero in the context of
    /// error checking.
    fn is_err_ok(self) -> bool { self > Self::epsilon() || -self > Self::epsilon() }
}
