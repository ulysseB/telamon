...
impl<'a> Cursor<'a> {
    ...
    fn select_action(&mut self, action: &Action) -> Result<(), ()> {
        let index = self
            ...
            .unwrap_or_else(|| {
                panic!(
                    "Unable to find action {} (available: {})",
                    action.display(self.node.candidate.ir_instance()),
                    self.node
                        .children
                        .iter()
                        .map(|e| e.action.display(self.node.candidate.ir_instance()))
                        .format(", ")
                )
            });
...
fn main() -> io::Result<()> {
    ...
    let (bundle, context) = context.kernel_bundle(&args.kernel);
    ...
    let check_fn = {
        ...
        move |_: &Candidate, context: &dyn Context| check_fn(context)
    };
    Evaluator::<Result<f64, String>>::scoped(
        ...
        EvalMode::FindBest,
        |evaluator| {
            let candidate = bundle.candidates[0].space.clone();
            let children = env
                .list_actions(&candidate)
                .into_iter()
                .map(|action| Edge {
                            ...
                            match key {
                                ...
                                Key::Char('r') => {
                                    let ref_runtime = Bench::default()
                                        .warmup(4)
                                        .runs(40)
                                        .benchmark_fn(&*bundle.reference_fns[0]);
                                    widget.actionline = Some(format!(
                                        "reference runtime: {}",
