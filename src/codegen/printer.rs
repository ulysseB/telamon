...
pub trait Printer {
    ...
    // TODO(cleanup): remove this function once values are preprocessed by codegen.
    // If values are preprocessed, types will be already lowered.
    ...
    /// Change the side-effect guards so that only the specified threads are
    /// enabled.
        ...
        match dim.kind() {
            ...
            DimKind::VECTOR => match *cfgs {
                [Cfg::Instruction(ref inst)] => {
                    self.print_vector_inst(inst, dim, namer, fun)
                }
    ...
    /// Prints a classic loop - that is, a sequential loop with an index and a
    /// jump to the beginning at the end of the block
    fn standard_loop(
        &mut self,
        fun: &Function,
        dim: &Dimension,
    ...
    fn inst(&mut self, inst: &Instruction, namer: &mut NameMap, fun: &Function) {
        match *inst.operator() {
            op::BinOp(op, ref lhs, ref rhs, round) => self.print_binop(
                op,
                inst.t(),
                round,
