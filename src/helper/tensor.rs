...
impl<'a> TensorBuilder<'a> {
    ...
    /// Swap two dimensions in the memory layout of the tensor. Keeps the
...
where S: ScalarArgument
{
    ...
    /// Creates a `VirtualTensor` that contains the values of `self`, loaded in
    /// registers.
...
impl VirtualTensor {
    ...
    /// Creates an operand that yeilds the values of the tensor in the given
    /// loop nest.
    pub fn dim_map<'a>(
        &self,
        dims: &[&MetaDimension],
