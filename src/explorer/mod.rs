...
// * illegal actions should be forbidden by applying their inverse as soon as
// possible. * avoid one copy of the candidate by reusing previous one when
// applying a choice might   be beneficial.

/// Entry point of the exploration. This function returns the best candidate
/// that it has found in the given time (or at whatever point we decided to
/// stop the search - potentially after an exhaustive search)
pub fn find_best<'a>(
    config: &Config,
    context: &Context,
...
/// Same as `find_best`, but allows to specify pre-existing actions and also
/// returns the actionsfor the best candidate.
    ...
    match config.algorithm {
        ...
        config::SearchAlgorithm::BoundOrder => {
            ...
            for candidate in candidates {
                candidate_list.insert(candidate);
            }
...
/// Launch all threads needed for the search. wait for each one of them to
/// finish. Monitor is supposed to return the best candidate found
...
/// Defines the work that explorer threads will do in a closure that will be
/// passed to context.async_eval. Also defines a callback that will be executed
/// by the evaluator
