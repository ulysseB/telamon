...
impl TreeStats {
    ...
    fn candidate(&mut self, _actions: Vec<ActionEx>, _bound: f64) -> CandidateStats {
        CandidateStats {}
    }

    fn duration(&mut self, timing: Duration) {
...
impl<'a, 'b> Tree<'a, 'b> {
    ...
    /// Removes the dead ends along the given path. Assumes the path points to
    /// a dead-end. Updates bounds along the way.
    ...
    /// Descend in the tree and tries to reach a leaf from the given
    /// `DescendState`.
    fn descend(
        &self,
        context: &Context,
...
impl<'a, 'b> Store<'a> for Tree<'a, 'b> {
    ...
    fn probe(&self, context: &Context) {
        ...
        unwrap!(self.shared_tree.write()).probe(context, cut, &mut stats);
        /* println!("{} leaf nodes.", stats.leafs.len()); */
    }
}
...
impl<'a> SubTree<'a> {
    ...
    /// Trims the branch if it has with an evaluation time guaranteed to be
    /// worse than `cut`. Returns the childrens to trim if any,
    ...
    fn probe(&mut self, context: &Context, cut: f64, stats: &mut TreeStats) {
        match self {
            SubTree::InternalNode(node, _bound) => {
                for child in unwrap!(node.write()).children.iter_mut() {
                    child.probe(context, cut, stats);
                }
            }
            SubTree::UnexpandedNode(candidate) => {
...
enum DescendState<'a> {
    /// The descent reached an internal node with multiple children. Indicates
    /// if the children were created during the descent.
...
impl<'a> Children<'a> {
    /// Creates a new children containing the given candidates, if any. Only
    /// keeps candidates above the cut.
    ...
    /// Trim children (that is, replace with an empty `SubTree`) children whose
    /// bounds are higher than the cut. Also clean-up rewards.
    ...
    /// Descends one level in the tree and apply an action on the candidate
    /// reached to generate a candidate that is not owned by the tree.
    /// Assumes all the `Children` are either dead-ends or unexpanded
    /// nodes. Returns the index of the selected child, the generated
    /// `Candidate` or the current state of the exploration if the candidate
    /// could not be generated. Returns `None` if a dead-end is reached before
    /// we could descend.
    fn descend_noexpand(
        &mut self,
        config: &BanditConfig,
    ...
    /// Update a rewards list given a new value and the position where it was
    /// found returns true if the value was inserted in the node.
    ...
    /// Returns the tuple `(child_index, eval_index, value)` containing the
    /// position and the value of the biggest reward registered.
...
/// gives a "score" to a branch of the tree at a given node n_successes is the
/// number of successes of that branch (that is, the number of leaves that
/// belong to the THRESHOLD best of that node and which come from that
/// particular branch). * `n_branch_trials` is the number of trials of that
/// branch (both failed and succeeded), * `n_trials` is  the number of trials
/// of the node and k the number of branches in the   node.
fn heval(
    config: &BanditConfig,
    n_successes: usize,
