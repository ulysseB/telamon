...
// TODO(search_space): explore and lower loayouts directly from the regular
// actions.
...
/// Chooses an order between instructions and dimensions when multiple are
/// possible. The function assumes the order between dimensions is already
/// fixed.
// TODO(search_space): fix order has currently no effect. Should we remove it ?
// It is unused because inst-dim and dim-dim decisions are fixed by the
// explorer. We cannot make them free as we might end-up in a dead-end.
