//! Compute and represent local information on the different objects
//! representing of the IR.
...
pub struct LocalInfo {
    ...
    /// The pressure induced by a single iteration of each dimension and the
    /// exit latency of the loop.
...
impl LocalInfo {
    /// Compute the local information for the given search space, in the
    /// context.
...
)
{
    for &(dim, _) in indvar.dims() {
        ...
        let mut overhead = if dim_kind.intersects(DimKind::UNROLL | DimKind::LOOP) {
            // FIXME: do not add the latency if the induction level can statically
            // computed. This is the case when:
...
pub struct Nesting {
    ...
    /// Dimensions that should not take the current BB into account when
    /// processed.
    ...
    /// Number of threads that are not represented in the active dimensions of
    /// the block.
    pub num_unmapped_threads: u32,
    /// Maximal number of threads this block can be in, considering only outer
    /// and mapped out dimensions.
