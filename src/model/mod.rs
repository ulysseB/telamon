...
// TODO(model): One some instruction, the latency dependens on the operand
// position. TODO(model): Some instructions are divided into multiple
// sub-instructions. When adding ordering dependencies, this must be taken
// into account as the last sub-instruction must be issued for the execution
// to continue to the next macro-instruction. The latency between the two
// macro-instructions is thus non-zero. TODO(model): latency of induction
// variables  - take the latency to the origin into account
//  - take the latency of add/muls into account
// - FIXME: should not be taken into account for inner latency of unrolled
// loops * account for unrolled induction variables only in the hw pressure
// and not in the      iteration overhead => also check regular loops
// TODO(model): take syncthread overhead into account
// TODO(model): the block parallelism is overestimated because we do not
// account for the  number of registers used per threads.
// FIXME: to avoid error, distinguish the issue and consumption of instructions.
// For example, a loop might issue loads, but the loads can end after the end
// of the loop is issued. For this, either double the nodes or subtract the
// size of buffers to the next  issue.
...
/// Returns a lower bound on the execution time of all the implementation
/// candidates in `space`, when executed in `context`.
...
/// Populates the dependency maps and the levels with dependency edges and
/// back-edges.
fn populate(
    space: &SearchSpace,
    device: &Device,
...
/// Updates the dependency maps to account for the data dependencies to an
/// instruction.
...
)
{
    ...
    let (immediate_preds, predecessors, latency_to_exit);
    {
        ...
        // TODO(cc_perf): only predecessors that have an outgoing edge that is not
        // already accounted for by another predecessor in the current
        // dependency map should be considered.
...
{
    // TODO(cc_perf): only predecessors that have an outgoing edge that is not
    // already accounted for by another predecessor in the current dependency
