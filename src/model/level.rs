...
//!
//! A level is akin to a loop, with a body and an iteration count, but it can
//! represent the intersection of the body of multiple iteration dimensions.
//! This is necessary when the nesting order of two loops of sizes N and M is
//! left unspecified. Indeed, when considering the loops independently, we can
//! only consider the body is repeated n or m times while when considering the
//! intersection of the bodies, we consider it is repeated N x M times.
...
pub struct Level {
    ...
    /// Dependencies between iterations of the level. A dependency is
    /// represented by the code point ID where the edge originates and goes
    /// to and by its latency.
...
impl Level {
    /// Creates a level iterating on the given dimensions. If no dimension is
    /// given, the level containts the whole program.
    fn new(
        device: &Device,
        space: &SearchSpace,
...
/// Computes the `HwPressure` caused by the intersection of the bodies of the
/// given loops.
pub fn sum_pressure(
    device: &Device,
    space: &SearchSpace,
...
) -> HwPressure
{
    ...
    let mut pressure =
        HwPressure::min(dims.iter().map(|d| &local_info.dim_overhead[d].0))
    ...
    // Get the list of inner dimensions and inner dimensions on wich the pressure
    // is summed.
    ...
    for bb in inner_bbs {
        ...
        // not mapped to a dimension outside of the block. Predicated instructions
        // require special care as they are only active on the dimensions they
        // are nested on. Other threads just skip the instruction.
        if bound_level <= BottleneckLevel::Block {
...
) -> FastBound
{
    ...
    // Repeat the pressure by the number of iterations of the level and compute the
    // bound.
    let n_iters = dims
        .iter()
...
///
/// The idea is to ensure that each instruction is considered the right number
/// of times and that inner loops are applied before outer ones. For this, we
/// build the list of outer dimensions of each instruction or loops. For loops,
/// we include both the nesting with and without the loop.  We then build the
/// minimal dag for the order defined such as X < Y iff:
/// - nesting(X) < nesting(Y)
/// - forall z in Y\X, forall y in Y, z inner y
/// Each edge of the dag represents a level, appling the dimensions in the
/// difference between the nestings at each end of the edge.
pub fn generate(
    space: &SearchSpace,
    device: &Device,
    local_info: &LocalInfo,
...
/// Indicates how a the sequential dimensions of a level should be repeated in
/// the latency graph.
...
impl LevelDag {
    ...
    /// Adds a dependency to all nodes that where the given dimensions are
    /// processed.
