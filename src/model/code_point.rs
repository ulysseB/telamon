...
pub enum CodePoint {
    /// An instruction.
    Inst(ir::InstId),
    /// The entry in a set of loops, referenced by their index in the loop
    /// level order.
    LevelEntry(usize),
    /// The exit of a set of loops, referenced by their index in the loop level
    /// order.
    LevelExit(usize),
}

impl CodePoint {
    /// Returns the conditions on which the `CodePoint` is lesser and greater
    /// than another.
...
/// Exposes data dependencies between code points and provides a valid total
/// order between them.
#[derive(Debug)]
...
/// Generates a directed acyclic graph representing the ordering dependencies
/// between the code points.
...
/// `greater_cond` indicates respectively the conditions for `lhs` to be
/// considered lesser and greater than `rhs`. `if_equals` indicates the order
/// to return if `lhs` and `rhs` are equals.
fn convert_order(
    space: &SearchSpace,
    lhs: ir::BBId,
    rhs: ir::BBId,
