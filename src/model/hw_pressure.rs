...
/// A lower bound on the execution time, fast to clone but relying on
/// information internal to the performance model.
pub type FastBound = ExplainedBound<Rc<FastOrigin>>;

/// A lower bound on the execution time, with a detailed explanation of the
/// origin of the bound.
...
impl FastBound {
    ...
    /// Chains two bounds. `self` and `other` must be a latencies to and from
    /// `mid_point`.
    ...
    /// Converts the bound so it can be explained without access to the
    /// performance model internals.
    pub fn explain(
        &self,
        device: &Device,
        levels: &[Level],
    ...
    ///
    /// * `hw_parallelism` - the number of computations that can be performed
    /// in parallel. * `iterations` - the number of times the computation
    /// bounded by `self` is repeated. * `max_par` -  the maximum number of
    /// independent jobs that can be built to compute `iterations` times
    /// the computation bounded by self.
...
impl BottleneckLevel {
    /// Indicates if a dimension should be taken into account for the
    /// bottleneck level.
...
impl FastOrigin {
    /// Converts the origin so it does not use information internal to the
    /// performance model.
    fn explain(
        &self,
        device: &Device,
...
/// Converts a `CodePoint` based on level ids into a `Point`, based on
/// dimension ids.
...
/// The justification behind a lower bound. Is slower to handle than
/// `FastOrigin` but does not refer internal information.
#[derive(Clone)]
pub enum Origin {
    /// The bound is caused by the latency between two instructions.
    Latency,
...
impl Origin {
    /// Simplifies the bound and indicates and previous and succedings bounds
    /// should be trimmed.
...
impl HwPressure {
    ...
    /// Adds the pressure of another computation, repeated in parallel. Ignores
    /// the latency.
