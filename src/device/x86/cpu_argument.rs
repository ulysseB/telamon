...
impl CpuArray {
    pub fn new(len: usize) -> Self { CpuArray(Mutex::new(vec![0; len])) }
...
impl Argument for CpuArray {
    fn size(&self) -> Option<u32> { Some(self.size()) }
...
where T: ScalarArgument
{
    fn as_size(&self) -> Option<u32> { self.as_size() }
...
impl Argument for Box<CpuScalarArg> {
    fn size(&self) -> Option<u32> { self.as_size() }

    fn arg_lock(&self) -> ArgLock { ArgLock::Scalar(self.scal_raw_ptr()) }
}
