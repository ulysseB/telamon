...
/// Represents a value that can be used as a `Function` argument. Must ensures
/// the type is a scalar and does not contains any reference.
pub unsafe trait ScalarArgument:
    Sync + Send + Copy + PartialEq + Display + 'static
{
    /// Returns the argument interpreted as an iteration dimension size, if
    /// applicable.
...
pub trait ArrayArgument: Send + Sync {
    // TODO(cc_perf): return a `Cow` instead of a `Vec` to avoid copying when
    // testing on a local CPU.
    // TODO(cleanup): use a type parameter instead of casting objects into bytes.
    // For this we first need rust/issues/#44265 to be solved.
