...
impl CudaPrinter {
    ...
    ) -> String
    {
        let args_str = args
            .iter()
            .map(|x| Self::vec_inst_arg(x, namer))
            .collect_vec()
            .join(", ");
...
impl Printer for CudaPrinter {
    ...
    )
    {
        ...
        unwrap!(writeln!(
            ...
            "{}.{} {}, {}, {};",
            operator,
            Self::get_type(Self::get_inst_type(mul_mode, return_type)),
            return_id,
            lhs,
            rhs
        ));
    ...
    )
    {
        ...
        unwrap!(writeln!(
            ...
            "{}.{} {}, {}, {}, {};",
            operator,
            Self::get_type(Self::get_inst_type(mul_mode, ret_type)),
            return_id,
            mlhs,
            mrhs,
            arhs
    ...
    )
    {
        ...
        match *inst.operator() {
            op::Ld(_, ref addr, _) => {
                ...
                let args = [
                    VecInstArg::VecInst(inst, dim.id(), size),
                    VecInstArg::Addr(addr),
                ];
                unwrap!(writeln!(Self::assemble(
                    &operator,
                    None,
                    inst.t(),
                    &args,
            ...
            op::St(ref addr, ref val, ..) => {
                ...
                let operands = [
                    VecInstArg::Addr(addr),
                    VecInstArg::VecOp(val, dim.id(), size),
                ];
