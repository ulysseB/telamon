...
impl<'a, T> device::ArrayArgument for Array<'a, T> where T: device::ScalarArgument {
    ...
    fn write_i8(&self, _: &[i8]) { panic!("no instance of Array should exist") }
}
...
impl Executor {
    ...
    pub fn compile_ptx<'a>(&'a self, _: &str, _: usize) -> Module<'a> { match *self { } }
...
/// A CUDA module.
pub struct Module<'a> { executor: &'a Executor }

impl<'a> Module<'a> {
    /// Returns the `Kernel` with the given name.
    pub fn kernel<'b>(&'b self, _: &str) -> Kernel<'a>  where 'a: 'b {
        match *self.executor { }
    }
}

/// A CUDA kernel, ready to execute.
pub struct Kernel<'a> { executor: &'a Executor }

impl<'a> Kernel<'a> {
    /// Executes the `Kernel` and returns the execution time in number of cycles.
    pub fn execute(&self, _: &[u32; 3], _: &[u32; 3], _: &[&Argument])
        -> Result<u64, ()>
    {
        match *self.executor { }
    }
    ...
    pub fn blocks_per_smx(&self, _: &[u32; 3]) -> u32 { match *self.executor { } }
}
