...
use device;
use device::cuda::{api, Context, Gpu, JITDaemon, PerfCounterSet};
...
impl<'a, 'b> Kernel<'a, 'b> {
    ...
    /// Runs a kernel and returns the number of cycles it takes to execute in
    /// cycles.
    ...
    /// Runs a kernel and returns the number of cycles it takes to execute in
    /// nanoseconds, measured using cuda event rather than hardware
    /// counters.
...
impl<'a> ThunkArgs<'a> {
    ...
    /// Evaluates the execution time of the kernel usig events rather than
    /// hardware counter.
