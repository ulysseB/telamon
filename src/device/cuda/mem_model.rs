...
// TODO(model): the pressure changes depending on the list of outer dimensions.
// Try to take this into account be computing the pressure incrementatly when
// applying levels.

/// Result of the memory analysis for one instruction. Vector instructions are
/// considered as a single instance and predicated dimensions are not
/// considered to compute the average pressure.
#[derive(Debug)]
pub struct MemInfo {
...
// TODO(model): The model can decrease if the maximal number decreases: the
// replay assume a full wrap if possible. This is correct as if the wrap is not
// full the waste ratio will repeat the replay factor to achieve the same
// number. However, it makes debugging the performance model harder.
fn info(
    space: &SearchSpace,
    inst: &ir::Instruction,
...
) -> MemInfo
{
    ...
    // Handle the case where the last dimension may not be active. In that case we
    // also try without the dimension as considering it as a thread may
    // increase the pressure. Only the last dimension needs sepcial handling as
    // other dimensions are fully contained into a wrap.
    if thread_dims
        .last()
...
/// Returns the size and stride of thread dimensions for a tensor access
/// pattern and sort them in an optimal or better-than-optimal order. For two
/// dimensions `d0`, `d1` such that `d0.stride` < `d1.stride` and `such that,
/// d0` can be nested inside `d1` the order guarantees that `d0 < d1`.
...
/// Returns the thread dimensions that are mapped outside an instruction but
/// not active under this instruction. The returned boolean indicates if the
/// thread dimension cannot be mapped to an active dimension and if the
/// dimension is predicated.
...
/// Sort thread dimensions in an optimal or better-than-optimal order. The
/// order may not respect dependencies since we don't know the exact order and
/// it would be too costly to explore all of them (exponential). Instead we
/// compute the minimal number of inner thread dimension for each dimension and
/// ensure this amount is respected.
///
/// Because we only support tensor accesses, bigger strides are multiples of
/// smaller strides. Thus smaller stride will lead to less replays.
fn sort_thread_dims(
...
/// Returns the offset of memory accesses for each thread in a wrap. The offset
/// is relative to the access of the first thread.
...
{
    // We only need to account for hits on the first bank. Other banks will have a
    // smaller replay factor.
...
/// Compare the nesting of two dimension in the dynamic schedule. Yeilds a valid partial
/// order.
...
mod tests {
    ...
    /// Generates function with a load in two thread dimensions, with
    /// non-coalesced accessed on the first one.
    fn gen_function<'a>(
