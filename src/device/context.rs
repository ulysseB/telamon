...
pub trait Context: Sync {
    ...
    /// Returns the execution time of a fully specified implementation in
    /// nanoseconds.
    fn evaluate(&self, space: &Function, mode: EvalMode) -> Result<f64, ()>;
    /// Compiles and benchmarks a functions. As opposed to `Self::evaluate`,
    /// the measured time contains potential startup times.
    ...
    /// `skip_bad_bounds` indicates than candidates whose bound is above
    /// the best candidate should be skiped.
    fn async_eval<'a, 'b>(
        &self,
        num_workers: usize,
        mode: EvalMode,
