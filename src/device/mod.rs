...
// TODO(perf): in PTX, shared and local pointers can have a 32-bit size, even
// in 64-bit mode. 32bits ops are potentialy faster than 64bits ops.
...
pub trait Device: Sync {
    ...
    /// Indicates if vectorization is possible on a loop with size Size on this
    /// instruction.
    ...
    /// Returns the pressure cause by a `BasicBlock`. For a dimension, returns
    /// the pressure for the full loop execution.
    fn hw_pressure(
        &self,
        space: &SearchSpace,
    ...
    /// Returns the pressure produced by a single iteration of a loop and the
    /// latency overhead of iterations.
    ...
    /// Returns the number of blocks that can be executed in parallel on the
    /// device.
    ...
    /// Returns the pressure caused by a multiplicative induction variable
    /// level.
    fn multiplicative_indvar_pressure(&self, t: &ir::Type) -> HwPressure;
    /// Adds the overhead (per instance) due to partial wraps and predicated
    /// dimensions to the pressure. If the instruction is not predicated,
    /// `predicated_dims_size` should be `1`.
