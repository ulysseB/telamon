...
impl<'a> Instruction<'a> {
    ...
    /// Indicates the operands for wich a `DimMap` must be lowered if lhs and
    /// rhs are not mapped.
    ...
    /// Returns 'true' if `self` is a reduction initialized by init, and if
    /// 'dim' should have the same nesting with 'init' that with 'self'.
    ...
    /// Adds a new iteration dimension. Indicates if the dimension was not
    /// already an iteration dimension.
