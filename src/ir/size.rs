...
impl<'a, T> std::ops::Mul<T> for Size<'a>
where
    T: Borrow<ir::Size<'a>>,
{
    type Output = Size<'a>;
...
impl<'a, 'b, T> std::ops::Mul<T> for &'b Size<'a>
where
    T: Borrow<ir::Size<'a>>,
{
    type Output = Size<'a>;
    ...
    fn mul(self, rhs: T) -> Self::Output {
        ...
        let params_factor = self
            .params_factor
            .iter()
            .chain(&rhs.params_factor)
            .cloned()
            .collect();
...
impl<'a, 'b> std::iter::Product<&'b ir::Size<'a>> for ir::Size<'a>
where
    'a: 'b,
{
    fn product<I>(iter: I) -> Self
    where
        I: Iterator<Item = &'b ir::Size<'a>>,
    {
