...
impl TypingContext {
    ...
    pub fn check_set_redefinition(&self, set_def: &SetDef) -> Result<(), TypeError> {
    ...
    pub fn check_set_parametric(&self, set_def: &SetDef) -> Result<(), TypeError> {
    ...
    pub fn check_superset_parametric(&self, set_def: &SetDef) -> Result<(), TypeError> {
    ...
    pub fn check_enum_parametric(&self, choice_def: &ChoiceDef) -> Result<(), TypeError> {
        if let ChoiceDef::EnumDef(enum_def) = choice_def {
...
impl TypingContext {
    ...
    ) -> Result<(), Spanned<TypeError>>
    {
        match statement {
            Spanned {
                ...
                data: d @ Statement::SetDef { .. },
            } => {
                let set_def: SetDef = Result::<SetDef, TypeError>::from(d)
                    .map_err(|e| Spanned { beg, end, data: e })?;

                self.check_set_redefinition(&set_def).map_err(|e| Spanned {
                    beg,
                    end,
                    data: e,
                })?;
                self.check_set_parametric(&set_def).map_err(|e| Spanned {
                    beg,
                    end,
                    data: e,
                })?;
            ...
            Spanned {
                ...
                data: d @ Statement::EnumDef { .. },
            }
            | Spanned {
                beg,
                end,
                data: d @ Statement::IntegerDef { .. },
            }
            | Spanned {
                beg,
                end,
                data: d @ Statement::CounterDef { .. },
            } => {
                let choice_def: ChoiceDef = Result::<ChoiceDef, TypeError>::from(d)
                    .map_err(|e| Spanned { beg, end, data: e })?;
    ...
    )
    {
        ...
        let trigger_code =
            print::add_to_quotient(set, &repr_name, &counter_name, &item_name, &arg_name);
    ...
    /// Creates a boolean choice that indicates if an object represents a givne
    /// class.
    ...
    /// Creates a counter for the number of objects that can represent another
    /// object in a quotient set. Returns the name of the counter.
    ...
    )
    {
        ...
        // Filter the counter itself after an update, because the filter actually acts
        // on the increments and depends on the counter value.
    ...
    /// Creates a choice to store the increment condition of a counter. Returns
    /// the corresponding choice instance from the point of view of the
    /// counter and the condition on wich the counter must be incremented.
    fn gen_increment(
        &mut self,
        counter: &str,
    ...
    /// Returns the `CounterVal` referencing a choice. Registers the
    /// UpdateCounter action so that the referencing counter is updated
    /// when the referenced counter is changed.
...
impl PartialEq for SetRef {
    fn eq(&self, rhs: &Self) -> bool { self.name == rhs.name }
}
...
impl PartialEq for VarDef {
    fn eq(&self, rhs: &Self) -> bool { self.set == rhs.set }
}
    ...
    pub fn check_symmetric(&self) -> Result<(), TypeError> {
        if self.statements.contains(&EnumStatement::Symmetric) {
            match self.variables.len() {
                ...
                _ => Err(TypeError::BadSymmetricArg(self.variables.to_owned()))?,
            }
        }
        Ok(())
    }
...
impl ChoiceDef {
    pub fn get_name(&self) -> &String {
        match self {
            ChoiceDef::CounterDef(counter_def) => &counter_def.name,
            ChoiceDef::EnumDef(enum_def) => &enum_def.name,
...
impl SetDef {
    ...
    /// A set should always have the keys: type, id_type, id_getter, iterator,
    /// getter.
    ...
    pub fn check_undefined_superset_key(&self) -> Result<(), TypeError> {
        if self.superset.is_some() {
            if self
                .keys
                .iter()
                .find(|(k, ..)| k == &ir::SetDefKey::FromSuperset)
                .is_none()
            {
                Err(TypeError::Undefined(
                    ir::SetDefKey::FromSuperset.to_string(),
                ))?
            }
        }
        Ok(())
