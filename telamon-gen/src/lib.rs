...
// TODO(cc_perf): Only iterate on the lower triangular part of symmetric
// domains in on_change
// TODO(cc_perf): in truth table, intersect rules with rules with weaker
// conditions,

// FIXME: fix counters:
// * Discard full counters. Might re-enable them later if we can find a way to
// make lowerings  commute
// * Fordid resrtricting the FALSE value of the repr flag from conditions that
// involve other decisions > this makes lowering commute. Otherwise we can
// force the counter to be >0, which can be true or not depending if another
// lowering has already occured. * Charaterise the effect of fragile values on
// performance FIXME: make sure the quotient set works correctly with things
// that force the repr flag to TRUE (for example the constraint on reduction).
// One problem might be that the flag is forced to FALSE but can be merged with
// a dim whose flag can be set to TRUE. The solutions for this may be to ensure:
