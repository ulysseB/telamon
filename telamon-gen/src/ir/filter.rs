...
#[derive(Debug, Clone)]
pub enum SubFilter {
    /// Enumerate the possible values that a input can take, and lists the
...
/// A list of constraints on the set each variable belongs to. It must be built
/// using `SetConstraints::new` so the constraints are in the right order.
...
impl SetConstraints {
    /// Create a new list of set constraints. The constraints are put in a
    /// normalized and legal order.
...
impl Adaptable for Variable {
    fn adapt(&self, adaptator: &ir::Adaptator) -> Self { adaptator.variable(*self) }
}
...
impl ChoiceInstance {
    /// Normalizes the `ChoiceInstance` and indicates if the corresponding
    /// input should be inversed.
...
#[derive(Clone, Debug, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub enum Condition {
    /// Triggers if the given piece of code returns `true`, given the mapping
    /// of variables.
    ...
    Bool(bool),
    /// Triggers if the one inputs respects the comparison operator with some
    /// rust code.
...
impl Condition {
    ...
    /// Returns allowed alternatives for the given input. Returns None if the
    /// condition is not on the given input.
    ...
    /// Returns the `StaticCond` representing the condition, if it exists.
...
pub enum ValueSet {
    ...
    Enum {
        ...
        values: BTreeSet<RcStr>,
        /// A set of inputs whose values can be take by the choice. Two flags
        /// indicate if the value should be negated or inversed.
...
impl ValueSet {
    ...
    pub fn empty(t: &ir::ValueType) -> Self {
        match t {
            ...
            | t @ ir::ValueType::NumericSet(..) => ValueSet::Integer {
                is_full: false,
    ...
    /// Indicates if the set contains all the values. This functions is
    /// pessimistic: the set may contain all the values and the function
    /// still return false.
    ...
    /// Intersects the `ValueSet` with the given values. Indicates if the
    /// intersection was successful or if the sets should be kept separate.
    ...
    /// Indicates if the `ValueSet` will be contained into anoter after
    /// instantiation. Requires both `self` and `other` to be instantiated.
