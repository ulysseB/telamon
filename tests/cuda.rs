...
use common::*;
use telamon::device::{ArrayArgument, Context, cuda};
...
fn global_vector_load() {
    ...
    input.1.as_ref().write(&(0..D0_LEN).map(|i| i as i32+10).collect_vec()[..]);
    ...
    check_candidates(builder.get(), &context, || {
        let res = output.1.as_ref().read::<i32>()[0];
        assert_eq!(res, 13);
    });
}
